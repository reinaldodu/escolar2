...
if(!empty($_POST))
{
    if(empty($_POST['nombre']) || empty($_FILES['file']['name'])) {
        ...
        $directorio = '../../images/materias/';
        if(!file_exists(($directorio))){
            mkdir($directorio, 0777, true);
        }
        //validar que el archivo sea de tipo imagen
        if ($type != 'image/jpg' && $type != 'image/jpeg' && $type != 'image/png' && $type != 'image/gif') {
            $respuesta = array('status' => false,'msg' => 'Solo se permiten archivos de tipo imagen');
        } else {
            ...
            if($result > 0) {
                ...
                if($idmateria < 0) {
                    ...
                    //consultar la extension del archivo
                    $ext = pathinfo($_FILES['file']['name'], PATHINFO_EXTENSION);
                    $destino = $destino = $directorio . $idmateria . '.'. $ext;
